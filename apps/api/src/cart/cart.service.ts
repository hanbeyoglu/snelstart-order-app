...
export class CartService {
  ...
  private calculateTotals(items: CartItem[]) {
    const subtotalExclVat = this.money(
    ...
    for (const item of items) {
      ...
      item.lineTotalInclVat = this.money(lineSubtotal + vatAmount);
      item.totalInclVat = item.lineTotalInclVat;
      ...
      current.totalInclVat = this.money(current.totalInclVat + item.totalInclVat);
