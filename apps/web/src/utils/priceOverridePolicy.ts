/** Browser-safe copy of packages/shared price-override logic (avoids CJS shared bundle). */
...
export function resolvePriceOverridePolicy(user: PriceOverrideUserContext): PriceOverridePolicy {
  ...
  if (permissions.includes(PRICE_OVERRIDE_PERMISSIONS.limited)) {
    const limitPercent =
      normalizeLimitPercent(user.priceOverrideLimitPercent) ?? DEFAULT_PRICE_OVERRIDE_LIMIT_PERCENT;
