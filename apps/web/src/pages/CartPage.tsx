...
export default function CartPage() {
  ...
  // Toplamları hesapla - fiyatlar KDV hariç, KDV satır bazında eklenir.
  const cartTotals = useMemo(() => {
    const vatByRate = new Map<number, { vatRate: number; subtotalExclVat: number; vatAmount: number; totalInclVat: number }>();
    ...
    for (const item of items) {
      ...
      const vatAmount = money((lineSubtotal * vatRate) / 100);
      const current = vatByRate.get(vatRate) || {
        vatRate,
        subtotalExclVat: 0,
        vatAmount: 0,
        totalInclVat: 0,
      };
      current.subtotalExclVat = money(current.subtotalExclVat + lineSubtotal);
      current.vatAmount = money(current.vatAmount + vatAmount);
      current.totalInclVat = money(current.totalInclVat + lineSubtotal + vatAmount);
      vatByRate.set(vatRate, current);
    }

    const vatBreakdown = Array.from(vatByRate.values()).sort((a, b) => a.vatRate - b.vatRate);
    const vatAmount = money(vatBreakdown.reduce((sum, item) => sum + item.vatAmount, 0));
        ...
        >
          ...
          {cartTotals.vatBreakdown
            ...
            return (
              ...
              >
                <span>{t('cart:vatRate', { rate: vatRate })}</span>
                <span>{formatCurrency(vatLine?.vatAmount || 0)}</span>
              </div>
            );
          })}
          <div
